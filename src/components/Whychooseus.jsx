...
/* ════════════════════════════════════════════════════════════
   SitesCarousel — True Owl Carousel: center card + side peeks
════════════════════════════════════════════════════════════ */
function SitesCarousel({ visible }) {
  ...
  // Card width = 82% of container, so 9% peeks on each side
  ...
  return (
    <div style={{
      ...
      {/* ── Owl viewport — overflow hidden, centers the active card ── */}
      <div
        style={{ overflow: "hidden", cursor: "grab", padding: "8px 0 12px" }}
        onPointerDown={onPointerDown}
        onPointerUp={onPointerUp}
      >
        {/*
          Track: each card is cardWidthPct% wide.
          translateX shifts by -(current * (cardWidthPct + gap%))
          We also add a left offset of 9% so first card is centred.
        */}
        <div style={{
          display: "flex",
          gap: `${gapPx}px`,
          // shift so active card is centred:
          // left padding = (100 - cardWidthPct) / 2 %
          paddingLeft: `${(100 - cardWidthPct) / 2}%`,
          paddingRight: `${(100 - cardWidthPct) / 2}%`,
          transform: `translateX(calc(-${current} * (${cardWidthPct}% + ${gapPx}px)))`,
          ...
          {CLIENT_SITES.map((site, i) => (
            <div
              ...
              style={{
                ...
                // Dim & scale down non-active cards
                opacity: i === current ? 1 : 0.38,
                transform: i === current ? "scale(1)" : "scale(0.93)",
                transition: "opacity 0.45s ease, transform 0.45s cubic-bezier(.22,1,.36,1)",
                pointerEvents: i === current ? "auto" : "none",
              }}
            >
              <SiteCard site={site} isActive={i === current} />
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            border: `1px solid ${current === 0 ? "rgba(37,135,168,0.15)" : "rgba(37,135,168,0.5)"}`,
            background: current === 0 ? "rgba(37,135,168,0.04)" : "rgba(37,135,168,0.12)",
            display: "flex", alignItems: "center", justifyContent: "center",
        ...
        >
          <svg viewBox="0 0 24 24" width="14" height="14" fill="none"
            stroke="#2587a8" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <polyline points="15 18 9 12 15 6"/>
          </svg>
        </button>
        ...
        <button
          ...
          style={{
            width: "36px", height: "36px", borderRadius: "50%",
            border: `1px solid ${current === total - 1 ? "rgba(37,135,168,0.15)" : "rgba(37,135,168,0.5)"}`,
            background: current === total - 1 ? "rgba(37,135,168,0.04)" : "rgba(37,135,168,0.12)",
            display: "flex", alignItems: "center", justifyContent: "center",
